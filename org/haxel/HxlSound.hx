 ...
 * @author	Adam Atomic
 */
class HxlSound extends HxlBasic
{
	/**
	...
	/**
	 * An public function for clearing all the variables used by sounds.
	 */
	...
	override public function update():Void
	{
		...
		if(_target != null)
		{
			radial = HxlU.getDistance(new HxlPoint(_target.x,_target.y),new HxlPoint(x,y))/_radius;
	 ...
	 * @param	AutoDestroy		Whether or not this <code>HxlSound</code> instance should be destroyed when the sound finishes playing.  Default value is false, but HxlG.play() and HxlG.stream() will set it to true by default.
	 * 
	 * @return	This <code>HxlSound</code> instance (nice for chaining stuff together, if you're into that).
	 */
	public function loadEmbedded(EmbeddedSound:Class<Sound>, Looped:Bool=false, AutoDestroy:Bool=false):HxlSound
	{
		stop();
		createSound();
	 ...
	 * @return	This <code>HxlSound</code> instance (nice for chaining stuff together, if you're into that).
	 */
	public function loadStream(SoundURL:String, Looped:Bool=false, AutoDestroy:Bool=false):HxlSound
	{
		stop();
		createSound();
		_sound = new Sound();
	 ...
	 */
	public function proximity(X:Float,Y:Float,Object:HxlObject,Radius:Float,Pan:Bool=true):HxlSound
	{
		x = X;
		y = Y;
		_target = Object;
	...
	{
		_transform.volume = (HxlG.mute?0:1)*HxlG.volume*_volume*_volumeAdjust;
		if(_channel != null)
			_channel.soundTransform = _transform;
	}
	
	/**
	 * An public helper function used to help Flash resume playing a looped sound.
	 * 
	...
	/**
	 * An public helper function used to help Flash clean up and re-use finished sounds.
	 * 
	 * @param	event		An <code>Event</code> object.
	 */
