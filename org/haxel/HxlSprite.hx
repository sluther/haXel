package org.haxel;
	...
	import org.haxel.system.HxlAnim;
	
	/**
	 * The main "game object" class, the sprite is a <code>HxlObject</code>
	...
	class HxlSprite extends HxlObject
	{
		...
		*/
		...
		/**
		 * Creates a white 8x8 square <code>HxlSprite</code> at the specified position.
		 ...
		 */
		public function loadGraphic(Graphic:Class,Animated:Bool=false,Reverse:Bool=false,Width:UInt=0,Height:UInt=0,Unique:Bool=false):HxlSprite
		{
			_bakedRotation = 0;
			_pixels = HxlG.addBitmap(Graphic,Reverse,Unique);
			if(Reverse)
		 ...
		 * @return	This HxlSprite instance (nice for chaining stuff together, if you're into that).
		 */
		public function loadRotatedGraphic(Graphic:Class, Rotations:UInt=16, Frame:Int=-1, AntiAliasing:Bool=false, AutoBuffer:Bool=false):HxlSprite
		{
			//Create the brush and canvas
			var rows:UInt = Math.sqrt(Rotations);
			var brush:BitmapData = HxlG.addBitmap(Graphic);
			if(Frame >= 0)
			{
			...
			if(AutoBuffer)
				max *= 1.5;
			var columns:UInt = HxlU.ceil(Rotations/rows);
			width = max*columns;
			height = max*rows;
			var key:String = String(Graphic) + ":" + Frame + ":" + width + "x" + height;
			var skipGen:Bool = HxlG.checkBitmapCache(key);
			_pixels = HxlG.createBitmap(width, height, 0, true, key);
		 ...
		 * @return	This HxlSprite instance (nice for chaining stuff together, if you're into that).
		 */
		public function makeGraphic(Width:UInt,Height:UInt,Color:UInt=0xffffffff,Unique:Bool=false,Key:String=null):HxlSprite
		{
			_bakedRotation = 0;
			_pixels = HxlG.createBitmap(Width,Height,Color,Unique,Key);
			width = frameWidth = _pixels.width;
			height = frameHeight = _pixels.height;
			resetHelpers();
			return this;
		 ...
		 */
		public function stamp(Brush:HxlSprite,X:Int=0,Y:Int=0):Void
		{
			Brush.drawFrame();
		...
		public function drawLine(StartX:Float,StartY:Float,EndX:Float,EndY:Float,Color:UInt,Thickness:UInt=1):Void
		{
			...
			var alphaComponent:Float = Number((Color >> 24) & 0xFF) / 255;
			if(alphaComponent <= 0)
				alphaComponent = 1;
			...
			_pixels.draw(HxlG.flashGfxSprite);
			dirty = true;
		}
		 ...
		 * This function is called automatically by <code>HxlSprite.postUpdate()</code>.
		 */
		 ...
		 */
		public function addAnimation(Name:String, Frames:Array, FrameRate:Float=0, Looped:Bool=true):Void
		{
		...
		public function play(AnimName:String,Force:Bool=false):Void
		{
			...
			HxlG.log("WARNING: No animation called \""+AnimName+"\"");
		}
		...
		{
			_curAnim = null;
			_curIndex = int(HxlG.random()*(_pixels.width/frameWidth));
			dirty = true;
		}
		...
		public function replaceColor(Color:UInt,NewColor:UInt,FetchPositions:Bool=false):Array
		{
			...
			while(row < rows)
			{
				...
				while(column < columns)
				{
					if(_pixels.getPixel32(column,row) == Color)
					{
						...
						if(FetchPositions)
							positions.push(new HxlPoint(column,row));
						dirty = true;
					}
					column++;
				}
		 ...
		 * Differs from <code>HxlObject</code>'s implementation
		 ...
		 */
		override public function onScreen(Camera:HxlCamera=null):Bool
		{
			if(Camera == null)
				Camera = HxlG.camera;
		...
		/**
		 * Checks to see if a point in 2D world space overlaps this <code>HxlSprite</code> object's current displayed pixels.
		 ...
		 */
		public function pixelsOverlapPoint(Point:HxlPoint,Mask:UInt=0xFF,Camera:HxlCamera=null):Bool
		{
			if(Camera == null)
				Camera = HxlG.camera;
			getScreenXY(_point,Camera);
