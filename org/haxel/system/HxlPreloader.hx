package org.haxel.system;
	...
	class HxlPreloader extends MovieClip
	{
		...
		[Embed(source="../data/logo_light.png")] private var ImgLogoLight:Class;
		...
		public function new()
		{
            ...
            {
                var re:RegExp = /\[.*:[0-9]+\]/;
                HxlG.debug = re.test(E.getStackTrace());
            }
			
			var tmp:Bitmap;
			if(!HxlG.debug && (myURL != null) && (root.loaderInfo.url.indexOf(myURL) < 0))
			{
		...
		private function update(Percent:Float):Void
		{
			...
			_text.text = "FLX v"+HxlG.LIBRARY_MAJOR_VERSION+"."+HxlG.LIBRARY_MINOR_VERSION+" "+Math.floor(Percent*100)+"%";
