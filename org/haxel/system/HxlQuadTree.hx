...
class HxlQuadTree extends HxlRect
{
	 ...
	 * Internal, used during tree processing and overlap checks.
	 */
	private var _processingCallback:Dynamic;
	/**
	 * Internal, used during tree processing and overlap checks.
	 */
	private var _notifyCallback:Dynamic;
	/**
	 * Internal, used during tree processing and overlap checks.
	 */
	 ...
	 */
	public function load(ObjectOrGroup1:HxlBasic, ObjectOrGroup2:HxlBasic=null, NotifyCallback:Dynamic=null, ProcessCallback:Dynamic=null):Void
	{
	...
	public function add(ObjectOrGroup:HxlBasic, List:UInt):Void
	{
		...
		if(Std.is(ObjectOrGroup, HxlGroup))
		{
			var i:UInt = 0;
			var basic:HxlBasic;
			var members:Array<HxlGroup> = cast(ObjectOrGroup, HxlGroup).members;
			var l:UInt = cast(ObjectOrGroup, HxlGroup).length;
			while(i < l)
			{
				basic = cast(members[i++], HxlBasic);
				if((basic != null) && basic.exists)
				{
					if(Std.is(basic, HxlGroup))
						add(basic,List);
					else if(Std.is(basic, HxlObject))
					{
						_object = cast(basic, HxlObject);
		...
		{
			_object = cast(ObjectOrGroup, HxlObject);
			if(_object.exists && _object.allowCollisions)
			{
				_objectLeftEdge = _object.x;
				_objectTopEdge = _object.y;
	...
	private function overlapNode():Bool
	{
		...
		while(_iterator != null)
		{
			...
			if((Type.getClass(_object) == Type.getClass(checkObject)) || !checkObject.exists || (checkObject.allowCollisions <= 0))
			{
