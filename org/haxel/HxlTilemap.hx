package org.haxel;
	...
	class HxlTilemap extends HxlObject
	{
		...
		override public function destroy():Void
		{
			...
			while(i < l)
				(_tileObjects[i++] as HxlTile).destroy();
			_tileObjects = null;
			i = 0;
			l = _buffers.length;
			while(i < l)
				(_buffers[i++] as HxlTilemapBuffer).destroy();
			_buffers = null;
			_data = null;
			_rects = null;
			_debugTileNotSolid = null;
		 ...
		 */
		public function loadMap(MapData:String, TileGraphic:Class, TileWidth:UInt=0, TileHeight:UInt=0, AutoTile:UInt=OFF, StartingIndex:UInt=0, DrawIndex:UInt=1, CollideIndex:UInt=1):HxlTilemap
		{
			auto = AutoTile;
		...
		override public function draw():Void
		{
			...
			while(i < l)
			{
				...
				buffer = _buffers[i++] as HxlTilemapBuffer;
				if(!buffer.dirty)
				{
		 ...
		 */
		public function getData(Simple:Bool=false):Array
		{
			if(!Simple)
				return _data;
			...
			while(i < l)
			{
				data[i] = ((_tileObjects[_data[i]] as HxlTile).allowCollisions > 0)?1:0;
				i++;
			}
			return data;
		}
		...
		public function setDirty(Dirty:Bool=true):Void
		{
			...
			while(i < l)
				(_buffers[i++] as HxlTilemapBuffer).dirty = Dirty;
		}
		...
		public function findPath(Start:HxlPoint,End:HxlPoint,Simplify:Bool=true,RaySimplify:Bool=false):HxlPath
		{
			...
			if( ((_tileObjects[_data[startIndex]] as HxlTile).allowCollisions > 0) ||
				((_tileObjects[_data[endIndex]] as HxlTile).allowCollisions > 0) )
				return null;
			...
			var node:HxlPoint;
			node = points[points.length-1] as HxlPoint;
			node.x = Start.x;
			node.y = Start.y;
			node = points[0] as HxlPoint;
			node.x = End.x;
			node.y = End.y;
		 ...
		 */
		private function simplifyPath(Points:Array):Void
		{
		 ...
		 * @param	Points		An array of <code>HxlPoint</code> nodes.
		 */
		private function raySimplifyPath(Points:Array):Void
		{
		 ...
		 */
		private function computePathDistance(StartIndex:UInt, EndIndex:UInt):Array
		{
			...
			while(i < mapSize)
			{
				if((_tileObjects[_data[i]] as HxlTile).allowCollisions)
					distances[i] = -2;
				else
					distances[i] = -1;
				i++;
		 ...
		 */
		private function walkPath(Data:Array,Start:UInt,Points:Array):Void
		{
		...
		override public function overlaps(ObjectOrGroup:HxlBasic,InScreenSpace:Bool=false,Camera:HxlCamera=null):Bool
		{
			if(ObjectOrGroup is HxlGroup)
			{
				...
				while(i < length)
				{
					...
					if(basic is HxlObject)
					{
						if(overlapsWithCallback(basic as HxlObject))
							results = true;
					}
					else
					{
			...
			else if(ObjectOrGroup is HxlObject)
				return overlapsWithCallback(ObjectOrGroup as HxlObject);
			return false;
		}
		...
		override public function overlapsAt(X:Float,Y:Float,ObjectOrGroup:HxlBasic,InScreenSpace:Bool=false,Camera:HxlCamera=null):Bool
		{
			if(ObjectOrGroup is HxlGroup)
			{
				...
				while(i < length)
				{
					...
					if(basic is HxlObject)
					{
						_point.x = X;
						_point.y = Y;
						if(overlapsWithCallback(basic as HxlObject,null,false,_point))
							results = true;
					}
					else
					{
			...
			else if(ObjectOrGroup is HxlObject)
			{
				_point.x = X;
				_point.y = Y;
				return overlapsWithCallback(ObjectOrGroup as HxlObject,null,false,_point);
			}
			return false;
		}
		 ...
		 */
		public function overlapsWithCallback(Object:HxlObject,Callback:Function=null,FlipCallbackParams:Bool=false,Position:HxlPoint=null):Bool
		{
			var results:Bool = false;
			...
			while(row < selectionHeight)
			{
				...
				{
					overlapFound = false;
					tile = _tileObjects[_data[rowStart+column]] as HxlTile;
		...
		{
			if(!InScreenSpace)
				return (_tileObjects[_data[uint(uint((Point.y-y)/_tileHeight)*widthInTiles + (Point.x-x)/_tileWidth)]] as HxlTile).allowCollisions > 0;
			...
			return (_tileObjects[_data[uint(uint((Point.y-_point.y)/_tileHeight)*widthInTiles + (Point.x-_point.x)/_tileWidth)]] as HxlTile).allowCollisions > 0;
		}
		 ...
		 */
		public function getTileCoords(Index:UInt,Midpoint:Bool=true):Array
		{
			var array:Array = null;
		...
		public function setTileProperties(Tile:UInt,AllowCollisions:UInt=0x1111,Callback:Function=null,CallbackFilter:Class=null,Range:UInt=1):Void
		{
			...
			while(i < l)
			{
				tile = _tileObjects[i++] as HxlTile;
		...
		public function ray(Start:HxlPoint, End:HxlPoint, Result:HxlPoint=null, Resolution:Float=1):Bool
		{
			...
			while(i < steps)
			{
				...
				if((_tileObjects[_data[tileY*widthInTiles+tileX]] as HxlTile).allowCollisions)
				{
		 ...
		 */
		public static function arrayToCSV(Data:Array,Width:Int,Invert:Bool=false):String
		{
			var row:UInt = 0;
			var column:UInt;
			var csv:String;
		 ...
		 * @return	A comma-separated string containing the level data in a <code>HxlTilemap</code>-friendly format.
		 */
		public static function bitmapToCSV(bitmapData:BitmapData,Invert:Bool=false,Scale:UInt=1,ColorMap:Array=null):String
		{
		 ...
		 * @return	A comma-separated string containing the level data in a <code>HxlTilemap</code>-friendly format.
		 */
		public static function imageToCSV(ImageFile:Class,Invert:Bool=false,Scale:UInt=1):String
		{
		...
		{
			var tile:HxlTile = _tileObjects[_data[Index]] as HxlTile;
