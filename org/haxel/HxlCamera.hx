...
class HxlCamera extends HxlBasic
{
	...
	/**
	 * Tells the camera to follow this <code>HxlObject</code> object around.
	 */
	public var target:HxlObject;
	/**
	 ...
	 */
	public var deadzone:HxlRect;
	/**
	 * The edges of the camera's range, i.e. where to stop scrolling.
	 * Measured in game pixels and world coordinates.
	 */
	public var bounds:HxlRect;
	
	/**
	 * Stores the basic parallax scrolling values.
	 */
	public var scroll:HxlPoint;
	/**
	 * The actual bitmap data of the camera display itself.
	 */
	public var buffer:BitmapData;
	/**
	 * The natural background color of the camera. Defaults to HxlG.bgColor.
	 * NOTE: can be transparent for crazy FX!
	 */
	public var bgColor:UInt;
	/**
	 * Sometimes it's easier to just work with a <code>HxlSprite</code> than it is to work
	 * directly with the <code>BitmapData</code> buffer.  This sprite reference will
	 * allow you to do exactly that.
	 */
	public var screen:HxlSprite;
	 ...
	 * Internal, used to control the "shake" special effect.
	 */
	private var _fxShakeOffset:HxlPoint;
	/**
	 * Internal, used to control the "shake" special effect.
	 */
	...
	override public function update():Void
	{
		...
		{
			_fxFlashAlpha -= HxlG.elapsed/_fxFlashDuration;
		...
		{
			_fxFadeAlpha += HxlG.elapsed/_fxFadeDuration;
			if(_fxFadeAlpha >= 1.0)
			{
				_fxFadeAlpha = 1.0;
		...
		if(_fxShakeDuration > 0)
		{
			...
			else
			{
				if((_fxShakeDirection == SHAKE_BOTH_AXES) || (_fxShakeDirection == SHAKE_HORIZONTAL_ONLY))
					_fxShakeOffset.x = (HxlG.random()*_fxShakeIntensity*width*2-_fxShakeIntensity*width)*_zoom;
				if((_fxShakeDirection == SHAKE_BOTH_AXES) || (_fxShakeDirection == SHAKE_VERTICAL_ONLY))
					_fxShakeOffset.y = (HxlG.random()*_fxShakeIntensity*height*2-_fxShakeIntensity*height)*_zoom;
			}
		}
	}
	
	/**
	 * Tells this camera object what <code>HxlObject</code> to track.
	 * 
	 ...
	 */
	public function follow(Target:HxlObject, Style:UInt=STYLE_LOCKON):Void
	{
		target = Target;
		var helper:Float;
		switch(Style)
		{
			case STYLE_PLATFORMER:
				...
				deadzone = new HxlRect((width-w)/2,(height-h)/2 - h*0.25,w,h);
			case STYLE_TOPDOWN:
				helper = HxlU.max(width,height)/4;
				deadzone = new HxlRect((width-helper)/2,(height-helper)/2,helper,helper);
			case STYLE_TOPDOWN_TIGHT:
				helper = HxlU.max(width,height)/8;
				deadzone = new HxlRect((width-helper)/2,(height-helper)/2,helper,helper);
			case STYLE_LOCKON:
			default:
				deadzone = null;
	 ...
	 */
	public function focusOn(Point:HxlPoint):Void
	{
	...
	public function setBounds(X:Float=0, Y:Float=0, Width:Float=0, Height:Float=0, UpdateWorld:Bool=false):Void
	{
		...
		if(UpdateWorld)
			HxlG.worldBounds.copyFrom(bounds);
		update();
	}
	 ...
	 * 
	 * @return	A reference to this <code>HxlCamera</code> object.
	 */
	public function copyFrom(Camera:HxlCamera):HxlCamera
	{
	...
	{
		/**
		*  due to haXe getter and setter restrictions, 
		*  we can't accept Float arguments for the setter
		*  and return a HxlPoint for the getter, so we
		*  accept <code>HxlPoint</code>s instead.
		*/
	 ...
	 * @private
	 */
	public function setScale(Point:HxlPoint):HxlPoint
	{
		_flashSprite.scaleX = Point.x;
		_flashSprite.scaleY = Point.y;
	}
	 ...
	 * using the existing <code>HxlCamera</code> variables.
	 * 
