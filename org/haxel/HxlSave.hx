...
class HxlSave
{
	private static inline var SUCCESS:UInt = 0;
	private static inline var PENDING:UInt = 1;
	private static inline var ERROR:UInt = 2;
	/**
	...
	public function bind(Name:String):Bool
	{
		...
		{
			HxlG.log("ERROR: There was a problem binding to\nthe shared object data from HxlSave.");
			destroy();
			return false;
		}
	...
	private function onDone(Result:UInt):Bool
	{
		...
		{
			case PENDING:
				HxlG.log("FLIXEL: HxlSave is requesting extra storage space.");
				break;
			case ERROR:
				HxlG.log("ERROR: There was a problem flushing\nthe shared object data from HxlSave.");
				break;
			default:
				break;
		}
	...
	private function checkBinding():Bool
	{
		...
		{
			HxlG.log("FLIXEL: You must call HxlSave.bind()\nbefore you can read or write data.");
			return false;
		}
		return true;
	}
